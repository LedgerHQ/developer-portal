...
For the <Glossary word="clear signing"></Glossary> to be possible with your App when buying or selling NFTs, you will need to instantiate the const `resolution` with `nft` set to `true` and then pass it when requesting the signature.
...
// Get the serialized <Glossary word="transaction"></Glossary> as an hex string
