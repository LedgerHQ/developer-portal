...
- Docker
- An elf of the <Glossary word="embedded app"></Glossary> for Nano S (create a empty folder with like : `<device>/<firmware version>/<appName>` example `nanos/1.6.1/mycoin`. The build of your <Glossary word="embedded app"></Glossary> must have the following format: `app_VERSION.elf`, for example `app_1.2.3.elf`)
...
The bot will try to sign the <Glossary word="transaction"></Glossary> using instructions that you provided in `speculos-deviceActions.ts`

Eventually it will broadcast the <Glossary word="transaction"></Glossary> in the <Glossary word="blockchain"></Glossary> and wait for the `sync` to find the operation and its optimisic version.

## How to define a test

### speculos-deviceActions.ts

It is required to know every screen that your <Glossary word="device app"></Glossary> contains, it will use the `title` of the screen then optionally check if the `expectedValue` of that screen is what it expects, then eventually execute the `button` action.
...
const acceptTransaction: DeviceAction<Transaction, *> = deviceActionFlow({
  steps: [
    ...
    },
    {
      title: "Destination",
      button: "Rr",
      expectedValue: ({ <Glossary word="transaction"></Glossary> }) => <Glossary word="transaction"></Glossary>.recipient,
    },
    {
      title: "Accept",
      button: "LRlr",
...
- `test` : checks / expectations after the <Glossary word="transaction"></Glossary> execution
  ...
  mutations: [
    {
      ...
      transaction: ({ account, siblings, bridge, maxSpendable }) => {
        ...
        const <Glossary word="transaction"></Glossary> = bridge.createTransaction(account);
      ...
      test: ({ account, accountBeforeTransaction, <Glossary word="transaction"></Glossary>, status }) => {
        const subAccountId = <Glossary word="transaction"></Glossary>.subAccountId;
        ...
        botTest("subAccount balance moved with the <Glossary word="tx"></Glossary> status amount", () =>
...
```ts copy
    name: "delegate new validators",
      ...
      test: ({ account, <Glossary word="transaction"></Glossary> }) => {
        const { cosmosResources } = account as CosmosAccount;
        invariant(cosmosResources, "cosmos");
        <Glossary word="transaction"></Glossary>.validators.forEach(v => {
...
```ts copy
  ...
  test: ({ accountBeforeTransaction, account, <Glossary word="transaction"></Glossary> }): void => {
    ...
    const erc20accountBefore = accountBeforeTransaction.subAccounts?.find(
      s => s.id === <Glossary word="transaction"></Glossary>.subAccountId,
    );
    invariant(erc20accountBefore, "erc20 acc was here before");
    invariant(account.subAccounts, "sub accounts");
    const erc20account = account.subAccounts!.find(s => s.id === <Glossary word="transaction"></Glossary>.subAccountId);
    ...
    if (transaction.useAllAmount) {
      ...
      botTest("account balance moved with <Glossary word="tx"></Glossary> amount", () =>
...
✔️ <Glossary word="transaction"></Glossary> 
SEND MAX
...
-> <Glossary word="Backend"></Glossary> issue
...
LedgerAPI5xx: <Glossary word="API"></Glossary> HTTP 504
(totally spent 98s – ends at 2024-04-03T11:32:38.725Z)
``` 

### How to update required screens

Open `libs/coin-modules/coin-bitcoin/src/speculos-deviceActions.ts` and update screens according to what you see on the <Glossary word="device app"></Glossary>, if `expectedValue` does not make sense anymore just delete it.

You can do a test run to see what screens fail (but the best way is to directly compare with real <Glossary word="device app"></Glossary> screens).