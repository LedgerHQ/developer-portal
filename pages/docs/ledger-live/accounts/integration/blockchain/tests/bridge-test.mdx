...
- <Glossary word="Transaction"></Glossary> status
- Account synchronisation

The approach is to test transactions and accounts synchronisation for the different currencies supported in <Glossary word="Ledger Live"></Glossary>. The transactions are not Broadcasted to the <Glossary word="blockchain"></Glossary>. We just verify that the bridge behave properly regarding to a <Glossary word="tx"></Glossary> inputs (recipient address, amount, fee…).
...
By doing so we can ensure that the account synchronisation and the <Glossary word="transaction"></Glossary> status will always provide the same results.
...
Only verify that <Glossary word="transaction"></Glossary> status is OK, sign and broadcast are out of the scope of those tests.
...
When running the <Glossary word="integration"></Glossary> tests, new snapshots will be generated that you can commit as expected results for the tests.
