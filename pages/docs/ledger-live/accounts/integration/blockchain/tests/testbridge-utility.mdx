...
Any test that requires HTTP to work should be moved in an <Glossary word="integration"></Glossary> test using the file name convention `.integration.test.ts`. In the case of testing a coin <Glossary word="integration"></Glossary>, we usually name that file `bridge.integration.test.ts` and we recommend to use the `testBridge` utility.

The `testBridge` utility takes a "DatasetTest" as a parameter which will conduct many kind of <Glossary word="integration"></Glossary> tests.
...
You can also generate it with a Ledger device, with a seed that you want to freeze (meaning you don't want to do anymore <Glossary word="transaction"></Glossary> with that seed, or you will need to regenerate the snapshot everytime) and execute in the <Glossary word="CLI"></Glossary> the command:
...
You are free to define your own extra tests in `bridge.integration.test.ts` (or any other <Glossary word="integration"></Glossary> test file) for more advanced tests that would not be covered by the bridge generic tests.
...
The <Glossary word="transaction"></Glossary> tests simulate an object `Transaction` as input, and a `TransactionStatus` as an output that we compare with an expected status.
...
```ts copy
import <Glossary word="Transaction"></Glossary> from "./types";
