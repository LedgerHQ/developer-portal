...
You will need to provide a JS implementation to interact with your coin <Glossary word="embedded app"></Glossary> on a Ledger.
...
<Callout type="warning" emoji="⚠️">
  If the <code>display</code> argument in <code>getAddress</code> doesn't work, get in touch with us - a necessary change may need to be made to the Ledger <Glossary word="embedded app"></Glossary>.
</Callout>

<Callout>
  Arguments are provided as example, but try to follow as much as possible other implementations for easy <Glossary word="integration"></Glossary>. If stuck, find and read your coin's <Glossary word="embedded app"></Glossary> documentation (<code>doc/api.asc</code>).
</Callout>

Any features provided by the <Glossary word="embedded app"></Glossary> should be provided through these JS bindings, such as `getAppConfiguration` or any blockchain-specific capabilities.
...
export default class MyCoin {
  ...
  /**
   * Sign a MyCoin <Glossary word="transaction"></Glossary> with a given BIP 32 path
   *
   * @param path a path in BIP 32 format
   * @param message a raw hex string representing a serialized <Glossary word="transaction"></Glossary>.
...
```ts copy
 ...
 function signTransaction(myCoin, deviceData, nonce) {
     ...
     console.log('Sending <Glossary word="transaction"></Glossary> to device for approval...');
