 ...
 * @returns {Transaction}
 */
export const createTransaction = (): <Glossary word="Transaction"></Glossary> => ({
  family: "mycoin",
  mode: "send",
  amount: BigNumber(0),
  recipient: "",
...
export const updateTransaction = (
  t: <Glossary word="Transaction"></Glossary>,
...
In some cases, this <Glossary word="transaction"></Glossary> will need to be prepared to correctly check status (`prepareTransaction`), like fetching the network fees, transforming some parameters, or setting default values, ...
...
/**
 * Prepare <Glossary word="transaction"></Glossary> before checking status
 *
 * @param {Account} a
 * @param {Transaction} t
 */
...
- user have enough funds to pay <Glossary word="transaction"></Glossary> fees
- the amount is strictly positive (avoiding zero sends)
- the minimum balance or existential deposit is respected if relevant
- etc

This <Glossary word="validation"></Glossary> is done everytime the user updates the <Glossary word="transaction"></Glossary> (any input changes) to give him immediate and contextual feedback. The status object returned by the `getTransactionStatus` follows this definition:
...
- `estimatedFees: BigNumber`: estimated total fees the <Glossary word="tx"></Glossary> is going to cost (in the mainAccount currency)
...
`errors` and `warnings` are key - value (error) objects that would have for each input (in the user perspective) the eventual error or notice that has been detected. To each key then it is expected that an input in <Glossary word="Ledger Live"></Glossary> Desktop and Mobile will exists to display the error.
...
As seen in the previous section, `getTransactionStatus` deals with errors in a <Glossary word="Transaction"></Glossary>, because of a wrong user input not meeting the coin logic.
...
/**
 * MyCoin error thrown on a specifc check done on a <Glossary word="transaction"></Glossary> amount
 */
...
/**
 * Returns true if <Glossary word="transaction"></Glossary> amount is less than MAX AMOUNT and > 0
 *
...
/**
 * Returns <Glossary word="nonce"></Glossary> for an account
 *
 * @param {Account} a
 */
export const getNonce = (a: Account): number => {
  ...
  const <Glossary word="nonce"></Glossary> = Math.max(
...
The `Transaction` object is not exactly the <Glossary word="transaction"></Glossary> in the shape of the blockchain's protocol (which is generally serialized into a blob of bytes). So for convenience, you may implement a `buildTransaction` method to serialized it using <i>MyCoin</i> <Glossary word="SDK"></Glossary>, that could be reused for instance for estimating fees through the <Glossary word="API"></Glossary>.
...
export const buildTransaction = async (a: Account, t: Transaction) => {
  ...
  const <Glossary word="nonce"></Glossary> = getNonce(a);
...
This `buildTransaction` function would return an unsigned <Glossary word="transaction"></Glossary> blob that would be signed with the <i>MyCoin</i> App on device.
...
/**
 * Sign <Glossary word="Transaction"></Glossary> with Ledger hardware
 */
export const buildSignOperation =
  (
    signerContext: SignerContext<MyCoinSigner>,
  ): AccountBridge<Transaction, MyCoinAccount>["signOperation"] =>
  ({ account, deviceId, <Glossary word="transaction"></Glossary> }): Observable<SignOperationEvent> =>
    new Observable(o => {
      async function main() {
        o.next({
        ...
        const operation = buildOptimisticOperation(
          account,
          <Glossary word="transaction"></Glossary>,
          <Glossary word="transaction"></Glossary>.fees ?? BigNumber(0)
        );
...
This operation is an optimistic version of the `Operation` that would be displayed in the account history as a "Pending Operation" if the broadcast succeed. This pending operation is important to give feedback to the user, but may also be required for calculate the <Glossary word="nonce"></Glossary> (if relevant).

### Front-end helpers

Live Common is mainly dedicated to be used by <Glossary word="Ledger Live"></Glossary> front-ends (Desktop and Mobile), so it also contains utilities for react and displaying crypto-specific data.

#### Device transaction fields

When signing a <Glossary word="transaction"></Glossary>, the user is shown on his device all the parameters of this <Glossary word="transaction"></Glossary> through multiple screen, that he must check towards the value he entered, and compare with what <Glossary word="Ledger Live"></Glossary> is presenting.

The list of all displayed fields on device are provided by the `getDeviceTransactionConfig` function, which must return all <Glossary word="transaction"></Glossary> fields for a given <Glossary word="transaction"></Glossary>.
...
<Callout type="warning" emoji="⚠️">
  Since a not-well-informed user could be tricked to sign <Glossary word="transaction"></Glossary> with wrong recipients, we never show the <code>destination</code> fields in <Glossary word="Ledger Live"></Glossary> applications, in order for users to get used to always verify it externally.
</Callout>
...
Once the <Glossary word="transaction"></Glossary> is signed, it must be broadcasted to MyCoin network. This is pretty easy if you correctly wrapped your <Glossary word="API"></Glossary>.
...
Once the operation is synced from <i>MyCoin</i> <Glossary word="API"></Glossary>, the AccountBridge will remove this optimistic operation from the `pendingOperations`.
...
The maximum spendable amount is the total balance in an account that is available to send in a <Glossary word="transaction"></Glossary>. This amount is specific to <i>MyCoin</i>, so you will need to provide this value depending on the <Glossary word="transaction"></Glossary> the user want to send.
...
Here is an example for a fee fetched from network from an unsigned <Glossary word="transaction"></Glossary> (a bit like Polkadot), but you can also have specific calculation, with fee-per-byte value provided by the <Glossary word="blockchain"></Glossary>.
...
/**
 * Fetch the <Glossary word="transaction"></Glossary> fees for a transaction
 *
 * @param {Account} a
 * @param {Transaction} t
 */
...
Before being able to test a `send` operation with <Glossary word="CLI"></Glossary> you will need to bind arguments and infer a <Glossary word="transaction"></Glossary> from it.
Since we defined a "mode" field in the <Glossary word="transaction"></Glossary>, this will be the only argument that will be necessary to test a send.

`libs/coin-modules/coin-mycoin/src/test/cli.ts`:

```ts copy
import flatMap from "lodash/flatMap";
import type { <Glossary word="Transaction"></Glossary>, AccountLike } from "@ledgerhq/types-live";
...
function inferTransactions(
  transactions: Array<{ account: AccountLike, transaction: <Glossary word="Transaction"></Glossary> }>,
  opts: Object
): Transaction[] {
  return flatMap(transactions, ({ <Glossary word="transaction"></Glossary>, account }) => {
...
Of course if <i>MyCoin</i> has more complex transactions, you can add many arguments to <Glossary word="CLI"></Glossary>. You can also define you own <Glossary word="cli"></Glossary> commands for any specific data you would like to fetch.
