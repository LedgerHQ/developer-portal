...
# Migrating from the Platform API
  ...
  Platform <Glossary word="API"></Glossary>. Here is a list of those key differences:
  ...
  <Glossary word="UX"></Glossary> based on those.
  2. **Method permissions:** Wallet-API adds plenty of new features. Since they can be used freely by the <Glossary word="live app"></Glossary>, it
  ...
  use.
  4. **More coin family support:** Wallet-API implement more crypto currency families than the Platform <Glossary word="API"></Glossary>. Please
  ...
  The <Glossary word="manifest"></Glossary> format hasn't been updated, however, some unused fields are now being used. Here is a list of the
  required modifications:

  ### `apiVersion`

  This field need to be set at version `^2.0.0` to ensure the connected wallet will expose the Wallet-API. If a lower
  version is used, the Platform <Glossary word="API"></Glossary> will be exposed instead
  ...
  This field now specify which currencies will be used by the <Glossary word="live app"></Glossary>. From the point of view of the <Glossary word="live app"></Glossary>, the
  ...
  This field now specify which methods can be used by the <Glossary word="live app"></Glossary>. Trying to use a method that wasn't declared in this
  ...
  Live apps using the Platform <Glossary word="API"></Glossary> are using the `@ledgerhq/live-app-sdk` package. This package provided a vanilla JS
  ...
  sign a <Glossary word="transaction"></Glossary>, with **transaction.sign** or **transaction.signAndBroadcast**. The first one will trigger a
  signature flow and return a **signed transaction**, the second one will trigger a signature flow and broadcast the
  signed <Glossary word="transaction"></Glossary> in one go, returning the **transaction hash**.
  2. **Transports:** Transport is an interface for a simple connector allowing the <Glossary word="live app"></Glossary> to connect to the wallet.
  ...
  developers might want to manage the <Glossary word="integration"></Glossary> themselves, we now offer a react wrapper library in order facilitate
  Wallet-API <Glossary word="integration"></Glossary> in their website, we are now providing the `@ledgerhq/wallet-api-client-react` library. This
  library is exposing both a provider and a set of react hooks, facilitating <Glossary word="integration"></Glossary>.
