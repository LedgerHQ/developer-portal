...
This page documents how to link the local packages of the wallet-api monorepo in order to work with <Glossary word="Ledger Live"></Glossary> locally (or any other application using the Wallet-API) without having to deploy a new version to the npm registry.
