...
The Exchange Module facilitates various exchange operations such as starting the exchange process, completing swap, sell, and fund transactions using <Glossary word="Ledger Live"></Glossary>.
...
- `fromAccountId` (required): Identifier of the account used as a source for the <Glossary word="transaction"></Glossary>.
- `rawTransaction` (required): A RawTransaction object containing the <Glossary word="transaction"></Glossary> details.
...
- `swapId` (optional): Identifier of the <Glossary word="backend"></Glossary> swap used (required for `"SWAP"`).
- `rate` (optional): Swap rate used in the <Glossary word="transaction"></Glossary> (required for `"SWAP"`).
