...
In addition, you can filter the currencies accessible by setting the `"currencies"` property inside your <Glossary word="manifest"></Glossary>. See the families subsection for more information about currencies filtering.
...
Each object in the array represents a <Glossary word="cryptocurrency"></Glossary> or token and contains the following properties:

- `type`: Specifies whether the currency is a base <Glossary word="cryptocurrency"></Glossary> like Bitcoin or <Glossary word="Ethereum"></Glossary> (`CryptoCurrency`) or a token like an ERC20 token (`TokenCurrency`).

- `id`: A unique identifier for the currency.

- `ticker`: The <Glossary word="ticker"></Glossary> symbol of the currency.

- `name`: The full name of the currency.

- `family`: The <Glossary word="blockchain"></Glossary> family the currency belongs to, such as `bitcoin` or `ethereum`.
...
- `parent`: Specifies the parent <Glossary word="blockchain"></Glossary> of the token.

Keep in mind that the actual response from the `useCurrencies` hook might be extensive, especially if you set the `"currencies" `property of your <Glossary word="manifest"></Glossary> to `"*"`.
