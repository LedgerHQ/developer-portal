...
const WalletAPIWebview => {
  ({ <Glossary word="manifest"></Glossary> customHandlers} ) => {
    const { webviewRef } = useWebviewState();

    useWebView({ <Glossary word="manifest"></Glossary>, customHandlers}, webviewRef);
    return <webview />
  }
};
```
...
const { onMessage } = useWalletAPIServer({
  <Glossary word="manifest"></Glossary>,
  accounts,
  config,
  webviewHook,
  uiHook,
...
to setup <Glossary word="Ledger Live"></Glossary> Wallet/UI/Store callbacks.
