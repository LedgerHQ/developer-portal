...
In order to communicate with Ledger’s <Glossary word="backend"></Glossary>, you must provide standardised APIs for Ledger's SWAP aggregator services to call.
...
<details>
  <summary>Additional Details: POST /quote</summary>
    ...
    All fees are expressed in output currency. For example, during a ETH to USDT on <Glossary word="ethereum"></Glossary> swap, the fees are expressed in USDT on <Glossary word="ethereum"></Glossary>.
...
<details>
  <summary>Additional Details: POST /swap</summary>
    ...
    message NewTransactionResponse {
        ...
        string    device_transaction_id = 11; // Legacy <Glossary word="nonce"></Glossary>, no need to give a value
    ...
    - `device_transaction_id`: swap <Glossary word="transaction"></Glossary> <Glossary word="nonce"></Glossary> provided by client at initialization (must be set to value of `nonce` from the <Glossary word="API"></Glossary> request) - this is a legacy property, only used by historical parter
    - `device_transaction_id_ng`: swap <Glossary word="transaction"></Glossary> <Glossary word="nonce"></Glossary> provided by client at initialization (must be set to value of `nonce` from the <Glossary word="API"></Glossary> request)
...
- HTTP status code 404
- Endpoints: `/status`
- Description: When the swap <Glossary word="transaction"></Glossary> is not found.
