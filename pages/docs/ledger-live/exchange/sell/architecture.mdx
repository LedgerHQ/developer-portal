...
<b>Post-Approval:</b> Once the transaction is approved, the Ledger Backend informs the provider backend of the transaction's approval, providing a transactionId if necessary.

If the transaction is rejected, the provider is also informed.
