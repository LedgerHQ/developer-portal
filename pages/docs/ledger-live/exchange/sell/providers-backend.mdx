...
Example: let's say your support AAVE on Ethereum mainnet. The LedgerID for AAVE on Ethereum is `ethereum/erc20/aave` (as we can see [here](https://crypto-assets-service.api.ledger-test.com/v1/currencies?output=id,ticker,symbol,network&search=AAVE)). This means:
