...
On running `pnpm i`, MyCoin will be added in `apps/ledger-live-desktop/src/renderer/families/generated.ts` if it doesn’t already exist (as an example, [see Algorand](https://github.com/LedgerHQ/docs/tree/develop/apps/ledger-live-desktop/src/renderer/families/algorand)). 
...
In each folder (of each coin), you’ll find code related to the UI of that coin only. For example, [in the Polkadot family](https://github.com/LedgerHQ/docs/tree/develop/apps/ledger-live-desktop/src/renderer/families/polkadot), there are many examples of UI custom Components:
