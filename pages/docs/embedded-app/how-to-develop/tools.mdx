...
`Speculos` is an emulator allowing to run and test an embedded application without loading it on a physical Ledger
...
(through `Speculos`). It aims at simplifying writing generic communication with an embedded application and managing
...
[requirement](../how-to-deploy/requirements/development#manifest) for an application to be deployed).
