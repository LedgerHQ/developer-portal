...
At the time of this writing, the default link script provided by the SDK for the Ledger Nano S allocates 4.5 KiB of RAM for applications to use. This 4.5 KiB has to be enough to store all global non-const and [non-NVRAM](../develop/sdk/memory/persistent-storage/) variables as well as the call stack.
...
- There's a core fault on the device (perhaps due to a [misaligned memory access](../develop/sdk/memory/memory-alignment/) or an attempt to access restricted memory)
...
Please refer to the [alignment](../develop/sdk/memory/memory-alignment/) page for further information.
