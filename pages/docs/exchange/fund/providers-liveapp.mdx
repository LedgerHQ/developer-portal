...
As explained in the [Fund Integration](../fund) page, you need to modify your LiveApp with some particularities compared to the one that can be found in the Discover section.
...
* `getFundPayload`: callback method called by the fund process to retrieve the payload (see below)

The `getFundPayload` is a callback that during the Fund process will be called with a `nonce`.
This value is required to craft on your side the Fund Payload, which will be returned by the callback.
...
```js copy
  exchangeSDK.fund({
    ...
    amount: "0.8",
    feeStrategy: "SLOW",
    getFundPayload: (nonce, fundAddress, amount) => {
      // Crafting Payload...
      return {
...
The LiveApp URL format is already fixed and will depend on the information you provided inside your [LiveApp manifest](../../../../docs/discover/wallet-api/appendix/manifest) (ex: `ledgerlive://discover/swapprovidername`).
  ...
  * `account-id`: User’s selected account id, where the crypto he wants to fund is stored.
...
__Example__:
```
https://fund.provider.widget.com?[account-id]=ua11-…&[crypto-currency]=BTC&[fiat-currency]=EUR&[crypto-amount]=0.04
```
