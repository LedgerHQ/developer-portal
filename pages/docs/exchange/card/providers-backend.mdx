...
You will find all the information regarding the CARD endpoints here: https://exchange-integration-card.redoc.ly/, as well as additional details on some endpoints below.
</Callout>

There are 5 main endpoints needed for the card:
    - To get the list of available fiat, country, payment, method and amount: [/capabilities](https://exchange-integration-card.redoc.ly/#operation/getCapabilities).
    - To get the list of available crypto-currencies: [/crypto-currencies](https://exchange-integration-card.redoc.ly/#operation/getCryptoCurrencies).
    - To get the fiat amount that the user should expect -after all fees are deducted - as a payment for the given crypto amount: [/quotes](https://exchange-integration-card.redoc.ly/#operation/getQuote).
    - To create a signed fund binary payload: [/sell](https://exchange-integration-card.redoc.ly/#operation/postFund).
    - To query a sell status: [/status](https://exchange-integration-card.redoc.ly/#operation/getStatus).
