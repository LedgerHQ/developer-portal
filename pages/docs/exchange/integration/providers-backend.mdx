...
Ledger needs to know:
  - The base url of the API,
  - Any information about the provider authentication mechanism :
    - Authentication for the backend to use the api (token in header...)
    - How to send referral information through the widget

## Endpoints needed for Swap / Sell / Buy / Fund

In order to communicate with Ledger’s backend, you must provide standardised APIs for Ledger's SWAP / BUY / SELL aggregator services to call.
