...
Also if your backend needs to know transaction status from our system (eg. Transaction approved/reject by user device) we do provide optional endpoint [/transaction/\{sellId\}/status](https://exchange-integration-sell.redoc.ly/#operation/getStatusUpdate)
