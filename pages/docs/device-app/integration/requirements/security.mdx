...
When the display of those parameters (Token, smart contract management) is not possible, the <Glossary word="transaction"></Glossary> should be rejected by the device unless the user has acknowledged that they will be <Glossary word="blind signing"></Glossary>.

To implement this requirement it is recommended to have a setting menu with the possibility to enable/disable <Glossary word="blind signing"></Glossary>.

<Callout type="warning" emoji="⚠️">
    If <Glossary word="blind signing"></Glossary> is implemented, it must be **disabled** by default.
</Callout>
...
The security issues with a severity level higher than low must be fixed before going through the <Glossary word="security audit"></Glossary>. The quality issues with a level set to error must also be fixed.