...
Applications should delegate **Events** like **Button Push Events** to the Ledger OS <Glossary word="UX"></Glossary> (in this case, using `UX_BUTTON_PUSH_EVENT(...)`) instead of handling them directly, in case the Ledger OS <Glossary word="UX"></Glossary> needs to override the application's UI. If the event is consumed by the Ledger OS <Glossary word="UX"></Glossary> (for example, a pressed button while the user is unlocking the screen) then the event is not passed on to the application.
