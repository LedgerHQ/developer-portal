...
Imagine an app where you have a flow for a <Glossary word="transaction"></Glossary> signature.
...
This works for a <Glossary word="transaction"></Glossary> signature. But suppose there's an update to the protocol: now some transactions can also sign some arbitrary data. However, not all transactions are required to sign the arbitrary data, so you need two flows:
...
Great, this works. But imagine that advanced users in the community also wants to be able to see the <Glossary word="nonce"></Glossary> for the <Glossary word="transaction"></Glossary>. But they want it to be optional.

You then need a flow for the vanilla signature, as well as a flow for the vanilla signature where you show the <Glossary word="nonce"></Glossary>. You also need the option to display the <Glossary word="nonce"></Glossary> when there is arbitrary data to sign, so you **also** need the "data signature" flow **and** the "data signature + nonce" flow.
...
// The vanilla flow with a step to display the <Glossary word="nonce"></Glossary>.
...
// This is identical to the flow just above, except there is step to display the <Glossary word="nonce"></Glossary>.
...
Here the app has been updated to support the protocol and the advanced users in the community can display the <Glossary word="nonce"></Glossary>. But imagine a new upgrade to the protocol allows the fees to be paid by another user of the <Glossary word="blockchain"></Glossary>, called a relayer. Some transactions would need to **hide** the fees (displaying a fee of 0.000 is not an option because it would confuse users more than anything).
...
To fix this problem, you would need to define the `UX_FLOW` at runtime, cherry-picking which steps you wish to include depending on the details of our <Glossary word="transaction"></Glossary>.
...
// It does some weird stuff with the `G_ux` global which is defined by the <Glossary word="SDK"></Glossary>.
