...
A step is declared using one of the **macros provided by the SDK**. Here's a list of the **most used ones** (a more exhaustive list is available by looking at the <Glossary word="SDK"></Glossary> source code):
...
Here's the example of a flow to sign a <Glossary word="transaction"></Glossary>. We first display "Confirm address" along with a picture, then use `bnnn_paging` to display the address because it might not fit on a single screen. We then display the amount and the transactions fees, and finally add two callback steps: the first one to confirm, the second one to reject.
