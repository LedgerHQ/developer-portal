...
3. Ledger reviews the <Glossary word="security audit"></Glossary> report
4. Ledger publishes your app

<Callout type="warning" emoji="⚠️">
    The <Glossary word="security audit"></Glossary> is mandatory for an application to be available on our production providers (accessible by
    default from the LedgerLive). However it is possible to deploy without <Glossary word="security audit"></Glossary> on other providers, for test
...
To go through an external <Glossary word="security audit"></Glossary>, ensure your project fulfills the following conditions:

- Your <Glossary word="Device App"></Glossary> works with all our devices (Ledger Nano S, S Plus, X, Stax and Flex)
- Your <Glossary word="Device App"></Glossary> has been functionally validated by Ledger team

<Callout type="warning" emoji="⚠️">
    <ul>
        <li>- Do not start a <Glossary word="security audit"></Glossary> process if your <Glossary word="Device App"></Glossary> is not ready for all Ledger devices (Ledger Nano S, S Plus, X, Stax and Flex).</li>
        <li>- Your <Glossary word="Device App"></Glossary> must still be functional after the <Glossary word="security audit"></Glossary> </li>
    </ul>
</Callout>

## Security audit specifications

Ledger has established and made public a detailed specification of what needs to be checked during a <Glossary word="security audit"></Glossary> following Ledger's standards.
...
- If <Glossary word="blind signing"></Glossary> is implemented:
...
- The compilation logs review must ensure (can be reviewed through the <Glossary word="CI"></Glossary> results):
...
- Guidelines enforcer and build workflows must be integrated to the <Glossary word="device application"></Glossary>.
...
- Functional tests must be written to cover at least the nominal cases (features provided by the applications) and common errors (for example <Glossary word="clear signing"></Glossary> cancellation).
- All functional tests must succeed for all devices.
- <Glossary word="Fuzzing"></Glossary> targets should be implemented at least for parsers and critical functions that are easy to fuzz as it will facilitate the audit.
- <Glossary word="Ethereum"></Glossary> plugins handlers must be fuzzed (a generic target is available in the boilerplate).
- <Glossary word="Fuzzing"></Glossary> should be done using libfuzzer to integrate with ClusterFuzzLite as shown in the boilerplate apps and plugins.
...
- For C apps, `scan-build` must succeed without returning any defects (can be reviewed through the <Glossary word="CI"></Glossary> results).
...
- For Rust apps, `clippy` must succeed without returning any issue (can be reviewed through the <Glossary word="CI"></Glossary> results).
...
- Cryptographic primitives must not be re-implemented if they already exist in Ledger OS or the <Glossary word="SDK"></Glossary>.
- Cryptographic functions that can throw exceptions are deprecated and must not be used (can be reviewed through the <Glossary word="CI"></Glossary> results).
- Cryptographic helpers and ledger assertions provided by the <Glossary word="SDK"></Glossary> should be used to handle cryptographic operations.
...
- [ ] <Glossary word="Clear signing"></Glossary> correctly displays information and asks for user confirmation.
- [ ] <Glossary word="Blind signing"></Glossary> is disabled by default or does not exist (if it exists, it does apply for complex transactions but not to coin transfers or swaps).
- [ ] App workflow is considered secure and snapshots cover the nominal and error cases of app features.
- [ ] Boilerplate Makefiles and <Glossary word="SDK"></Glossary> are used to compile the app. If it's a new app, it has been developed over the C/Rust boilerplates.
...
- [ ] Unit tests, functional tests and <Glossary word="fuzzing"></Glossary> tests are passing.
- [ ] For app-plugins, the generic <Glossary word="fuzzing"></Glossary> target is integrated.
...
- [ ] The <Glossary word="review process"></Glossary> looked into technical vulnerabilities as well as logical ones and weaknesses.
...
- [ ] No cryptographic primitives are re-implemented if they already exist in Ledger OS or the <Glossary word="SDK"></Glossary>.
