...
Ledger devices are protected from interdiction attacks (being tampered with while en route from Ledger's warehouses to your home) due to anti-tampering technology built into the <Glossary word="firmware"></Glossary>. Using attestation, the authenticity of the device is verified in software every time you plug it into one of the <Glossary word="Ledger Live"></Glossary> applications.
...
It is extremely unlikely for the Device private key to become compromised, because the <Glossary word="Secure Element"></Glossary> is designed to be a stronghold against such physical attacks. It is theoretically possible to extract the private key, but only with great expense and time, so only an organization such as the NSA could do it.
