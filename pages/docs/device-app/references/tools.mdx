...
- Build the <Glossary word="device application"></Glossary> for all existing devices
...
Ledger provides several workflows to ease the <Glossary word="integration"></Glossary> of applications into the GitHub <Glossary word="CI"></Glossary>.
...
<Callout emoji="📖">
    ...
    Usage examples can be found in the <Glossary word="CI"></Glossary> of lots of applications, for instance the
...
`Speculos` is an emulator allowing to run and test an <Glossary word="device application"></Glossary> without loading it on a physical Ledger
...
(through `Speculos`). It aims at simplifying writing generic communication with an <Glossary word="device application"></Glossary> and managing
...
For now, it only contains a tool (`ledger-manifest`) allowing to check `ledger_app.toml` <Glossary word="manifest"></Glossary> files (which are a
...
<Callout type="warning" emoji="⚠️">
  A Ledger device with a custom CA installed can not pass the Ledger Genuine Check, which is required to install applications from the <Glossary word="Ledger Live"></Glossary>. To make it pass the check, uninstall your custom CA and all the applications installed through it.
</Callout>
