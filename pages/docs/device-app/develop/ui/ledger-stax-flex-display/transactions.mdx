...
## Fields details: adding context and info to addresses

You can add a second layer of information for address fields, by using a progressive disclosure.

Your users can tap on the whole row (not just the small arrow) associated with an address, to reveal more information:

![App architecture](/device-app/advanced-transaction.png)

Two use cases are currently supported
- ENS domains (shown above on Ledger Flex)
- and account labels, an upcoming feature tied to the Ledger Live Address Book (shown below on Ledger Stax),
with more use cases potentially added in the future.

![App architecture](/device-app/advanced-transaction.png)

<Callout emoji="💻">
	<b>API</b><br/>
	While the use-case API doesn’t change, each field has an <code>alias</code> parameter that can be enabled to display the fields details presented above.
</Callout>
