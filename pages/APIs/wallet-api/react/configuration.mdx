# Configuring the Ledger Wallet API

After installing the necessary packages for the Ledger Wallet API, the next step is to set up the appropriate configuration through a layer that will call "Transport".
...
With this setup, the entire application can access the Ledger Wallet API and communicate with it via the `TransportProvider`.
