...
As you may see in `libs/coin-mycoin/src/bridge/js.ts`, Live Common provides a helper to implement it easily with `makeAccountBridgeReceive()`, and there are a very few reasons to implement your own.
...
`libs/coin-mycoin/src/js-synchronisation.ts`:
...
In some cases, you might need to write a `postSync` patch to add update logic after sync (<i>before the reconciliation that occurs on Ledger Live Desktop</i>). If this `postSync` function is complex (eg, more than 200 lines of code), you should split it in a `libs/coin-mycoin/src/js-postSyncPatch.js` file.
...
`libs/coin-mycoin/src/preload.ts`:
